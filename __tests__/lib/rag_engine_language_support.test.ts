...
describe('RAG 엔진 다국어 지원 테스트', () => {
  describe('언어별 분석 결과 생성', () => {
    // 여러 언어로 테스트할 키워드 목록
    const testKeywords = ['인공지능', 'GPT', '머신러닝'];
    
    test('한국어(기본) 분석이 올바르게 생성되어야 함', () => {
      const analysis = generateKeywordAnalysis(testKeywords);
    ...
    test('영어 분석이 올바르게 생성되어야 함', () => {
      const preferences: UserPreferences = {
        language: 'en'
      };
      
      const analysis = generateKeywordAnalysis(testKeywords, preferences);
  ...
  describe('다국어 사용자 맞춤 설정', () => {
    test('영어 설정에서 오류 메시지가 영어로 반환되어야 함', () => {
      const preferences: UserPreferences = {
        language: 'en'
      };
      
      // 빈 배열 전달 (오류 상황)
      const analysis = generateKeywordAnalysis([], preferences);
      
      // 영어 오류 메시지 확인
      expect(analysis).toBe('No keywords to analyze.');
    });
    
    test('영어 설정에서 에러 처리가 영어로 되어야 함', () => {
      const preferences: UserPreferences = {
        language: 'en'
      };
      
      // @ts-ignore - 의도적으로 잘못된 타입 전달
      const analysis = generateKeywordAnalysis('invalid input', preferences);
  ...
  describe('다국어 지원이 다양한 카테고리에 적용되는지 테스트', () => {
    const categories = [
    ...
    test.each(categories)('$name 카테고리의 영어 분석이 생성되어야 함', ({ keyword }) => {
      const preferences: UserPreferences = {
        language: 'en'
      };
      
      const analysis = generateKeywordAnalysis([keyword], preferences);
      
      // 영어 형식 확인
      expect(analysis).toContain(`## ${keyword} Keyword Analysis`);
      expect(analysis).toContain('### Key Insights');
      expect(analysis).toContain('### Content Strategy');
    });
  });
  ...
  describe('언어 전환 테스트', () => {
    test('동일한 키워드에 대해 언어별로 다른 결과가 생성되어야 함', () => {
      const keywords = ['기술 트렌드', 'technology', '미래 기술'];
      
      // 한국어 분석
      const koAnalysis = generateKeywordAnalysis(keywords, { language: 'ko' });
      
      // 영어 분석
      const enAnalysis = generateKeywordAnalysis(keywords, { language: 'en' });
