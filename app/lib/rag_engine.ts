...
function generateMarketingAnalysis(mainKeyword: string, keywords: string[]): string {
  ...
  const insights = [
    ...
    `${topKeywords[1] || topKeywords[0]} 관련 콘텐츠는 경쟁이 적은 편으로, 구체적인 방법론과 단계별 가이드를 제공하면 경쟁 우위를 점할 수 있습니다.`,
    `최근 6개월간 ${topKeywords[2] || topKeywords[0]}에 대한 검색이 45% 증가했으며, 성공 사례와 실패 사례를 모두 다루는 콘텐츠가 주목받고 있습니다.`,
  ];
...
function generateDevelopmentAnalysis(mainKeyword: string, keywords: string[]): string {
  ...
  // 안전하게 랜덤 키워드 선택
  const safeGetRandomKeyword = () => {
    if (keywords.length === 0) return mainKeyword;
    return keywords[Math.floor(Math.random() * keywords.length)];
  };
  
  const randomKeywords = [
    safeGetRandomKeyword(),
    safeGetRandomKeyword(),
    safeGetRandomKeyword()
  ];
...
function generateGenericAnalysis(mainKeyword: string, keywords: string[]): string {
  ...
  // 안전하게 키워드 배열 접근
  const getKeywordSafely = (index: number) => {
    if (index >= keywords.length) return mainKeyword;
    return keywords[index];
  };
  
  const insights = [
    `${getKeywordSafely(0)}에 대한 검색량이 가장 높으며, 사용자들은 주로 기본 개념과 사용 방법에 관심이 많습니다.`,
    `${getKeywordSafely(1)}와 ${getKeywordSafely(2)}는 상호 연관성이 높으며, 두 주제를 함께 다루는 콘텐츠가 효과적일 수 있습니다.`,
