...
export function generateKeywordAnalysis(
  ...
  try {
    ...
    // 캐시 키 생성 (키워드 + 설정 조합)
    const cacheKey = JSON.stringify({ keywords, preferences });
    ...
    for (let i = 0; i < insightCount; i++) {
      const insightText = generateInsight(category, language, i);
    ...
    for (let i = 0; i < strategyCount; i++) {
      const strategyText = generateStrategy(category, language, i);
      strategies.push(`${i + 1}. ${strategyText}`);
    }
 ...
 * @returns 인사이트 텍스트
 */
function generateInsight(category: KeywordCategory, language: SupportedLanguage, index: number): string {
  // 영어 인사이트
  const enInsights: Record<KeywordCategory, string[]> = {
    'AI 기술': [
  ...
  // 한국어 인사이트
  const koInsights: Record<KeywordCategory, string[]> = {
    'AI 기술': [
  ...
  const insights = language === 'en' ? enInsights : koInsights;
 ...
 * @returns 전략 텍스트
 */
function generateStrategy(category: KeywordCategory, language: SupportedLanguage, index: number): string {
  // 영어 전략
  const enStrategies: Record<KeywordCategory, string[]> = {
    'AI 기술': [
  ...
  // 한국어 전략
  const koStrategies: Record<KeywordCategory, string[]> = {
    'AI 기술': [
  ...
  // 언어에 따라 적절한 전략 배열 선택
  const strategies = language === 'en' ? enStrategies : koStrategies;
